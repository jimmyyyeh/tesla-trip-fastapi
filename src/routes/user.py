...
def verify(payload: Verify, db: Session = Depends(CRUD.get_db)):
...
def resend_verify(payload: ResendVerify, background_tasks: BackgroundTasks, db: Session = Depends(CRUD.get_db)):
...
def reset_password(payload: ResetPassword, db: Session = Depends(CRUD.get_db)):
